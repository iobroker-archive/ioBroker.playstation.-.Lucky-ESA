...
module.exports = {
    ...
    async updateProfile(constants, first) {
        ...
        url = `${constants.BASE_PATH.legacy_profile_uri.replace("{lang}", this.lang)}${constants.API_PATH.legacy_profile.replace("{online_id}", profile.data.onlineId)}`;
    ...
    async loadOnline_id(val, constants) {
        ...
        const url = `${constants.BASE_PATH.legacy_profile_uri.replace("{lang}", this.lang)}${constants.API_PATH.legacy_profile.replace("{online_id}", val)}`;
        ...
        if (status_user) {
            if (Object.keys(response).length > 0) {
                ...
                for (const id of response) {
                    ...
                    url = `${constants.BASE_PATH.legacy_profile_uri.replace("{lang}", this.lang)}${constants.API_PATH.legacy_profile.replace("{online_id}", onlineId)}`;
        ...
        const params = {
            params: {
                variables: JSON.stringify({
                    ...
                    displayTitleLocale: this.config.langPSN,
                }),

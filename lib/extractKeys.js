...
module.exports = class Json2iob {
    ...
    async parse(path, element, options) {
        ...
        try {
            ...
            path = path.replace(this.adapter.FORBIDDEN_CHARS, "_");
            ...
            for (const key of objectKeys) {
                ...
                const path_key = `${path}.${key}`.replace(this.adapter.FORBIDDEN_CHARS, "_");
    ...
    async extractArray(element, key, path, options) {
        ...
        try {
            if (key === "languagesUsed" || key === "languages") {
                await this.createLang(`${path}.${key}`, options);
                await this.parse(`${path}.${key}.${key}`, JSON.stringify(element[key]), options);
            }
            if (key) {
                element = element[key];
            }
            for (let index in element) {
                ...
                if (
                    ...
                    const key_subkey = `${path}.${subKey}`.replace(this.adapter.FORBIDDEN_CHARS, "_");

...
module.exports = {
    ...
    async updateProfile(constants, first) {
        ...
        if (legacy && legacy.data && legacy.data.profile) {
            if (legacy.data.profile.languagesUsed[0]) {
                this.lang = legacy.data.profile.languagesUsed[0];
            }
    ...
    async loadReceivedRequests(name, constants) {
        ...
        const methode = "get";
        const params = { params: { limit: limit.val } };
        ...
        if (status_user) {
            if (Object.keys(response).length > 0) {
                ...
                for (const id of response) {
                    ...
                    const user = await this.requestPSN(methode, url, header, params, true);
    ...
    async getUniversalSearch(name, platform, constants) {
        ...
        const resp = await this.requestPSN(methode, url, header, params, true);
        this.log.debug(`RESP: ${JSON.stringify(resp.data)}`);
        const obj = await this.getObjectAsync(`profile_remote.search_result_pagination`);
        if (resp && resp.data && resp.data.data) {
            await this.setState(`profile_remote.search_result`, { val: JSON.stringify(resp.data.data), ack: true });
            if (
                resp.data.data &&
                ...
                if (resp.data.data.universalContextSearch.results[0].next != "") {
                    ...
                    await this.setObjectAsync(`profile_remote.search_result_pagination`, obj);
                    return;
                }
            }
        } else {
            await this.setState(`profile_remote.search_result`, { val: JSON.stringify({}), ack: true });
        }
    ...
    async getHintAvailability(npCommId, constants) {
        ...
        this.log.debug(`RESP: ${JSON.stringify(resp.data)}`);
        if (resp && resp.data && resp.data.data) {
            await this.setState(`profile_remote.result`, { val: JSON.stringify(resp.data.data), ack: true });
        } else {
            await this.setState(`profile_remote.result`, { val: JSON.stringify({}), ack: true });
        }
    },
